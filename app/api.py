...
app = FastAPI(
    title="Moody Tweets",
    description="This API lets you make predictions about the sentiment (positive or negative) of a short text.",
    version="0.1",
)
...
def sentiment_prediction(request: Request, text: str = ""):
    '''Get a Sentiment Analysis on a text sequence performed by a deep learning model'''
    if text == "":
        # non valid input
        ...
        response = {
            "message": HTTPStatus.OK.phrase,
            "status-code": HTTPStatus.OK,
            "data": {"Sentiment prediction": prediction, "Model score" : float(x[0])},
        }
    return response
